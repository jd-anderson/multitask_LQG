...
def train_multitask_shared_Ktilde(tasks: List[LQGTask], p: int,
    ...
    Ktilde0      = Ktilde0 + 0.2 * rng.standard_normal(Ktilde0.shape)
...
# =============================================================
# 6b) Inverted pendulum system
# =============================================================

def build_inverted_pendulum(m: float, l: float, g: float = 9.81, dt: float = 0.05):
    """
    Single-link inverted pendulum linearised about the upright equilibrium.

    State:  x = [theta, theta_dot]   (angle from upright, angular velocity)
    Input:  u = torque at pivot
    Output: y = theta                (angle measured by encoder)

    Continuous-time:
        A_c = [[0, 1], [g/l, 0]]
        B_c = [[0], [1/(m*l^2)]]

    The system has one unstable discrete eigenvalue (pole above 1) and one
    stable eigenvalue (below 1), and is fully controllable and observable.
    ...
    A_d = np.eye(2) + dt * A_c
    B_d = dt * B_c
    C   = np.array([[1., 0.]])     # observe angle only
    return A_d, B_d, C


def make_pendulum_tasks(M: int = 4, seed: int = 0, perturb: float = 0.05,
                        m: float = 0.5, l: float = 0.3,
                        g: float = 9.81, dt: float = 0.05) -> List[LQGTask]:
    """
    Generate M inverted-pendulum LQG tasks by sampling m and l uniformly
    from [nominal*(1-perturb), nominal*(1+perturb)].

    Cost and noise matrices are the same as for the cartpole:
        Qy = 0.1*I,  R = 0.1*I,  W = 0.02*I,  V = 0.05*I

    All tasks are verified to have a finite Kalman DARE solution before
    being accepted.
    """
    ...
    while len(tasks) < M and tries < M * 50:
        tries += 1
        m_i = max(1e-3, m * (1 + perturb * rng.standard_normal()))
        l_i = max(1e-3, l * (1 + perturb * rng.standard_normal()))
        A, B, C = build_inverted_pendulum(m_i, l_i, g=g, dt=dt)
        nx, nu, ny = A.shape[0], B.shape[1], C.shape[0]
        Qy = 0.1 * np.eye(ny)
        R  = 0.1 * np.eye(nu)
        W  = 0.02 * np.eye(nx)
    ...
    if len(tasks) < M:
        raise RuntimeError(
            f"Could not generate {M} feasible pendulum tasks. "
            "Try reducing perturb.")
    return tasks


# =============================================================
# 6c) Synthetic LTI system
# =============================================================

def make_synthetic_tasks(M: int = 4, seed: int = 0, perturb: float = 0.05,
                         nx: int = 4, nu: int = 2, ny: int = 2,
                         dt: float = 0.05) -> List[LQGTask]:
    """
    Generate M synthetic LTI LQG tasks by perturbing a fixed nominal system.

    Nominal system design
    ---------------------
    The nominal A is chosen as a diagonal matrix with one unstable eigenvalue
    (1.1) and three stable ones, so the system is open-loop unstable but
    easily stabilisable.  B and C are chosen so that the system is both
    controllable and observable by construction:

        A_nom[i,i] = [1.1, 0.9, 0.85, 0.8]   (first entry > 1 = unstable)
        A_nom[i,i+1] = 0.1                    (coupling between modes)
        B_nom = [I_{nu}; 0]                   (first nu states actuated)
        C_nom = [I_{ny}, 0]                   (first ny states observed)

    Each task perturbs A and B by Gaussian noise scaled by `perturb`, and C
    by a much smaller perturbation (1% of perturb) to preserve observability.

    Every candidate task is verified for:
        (a) Full observability rank = nx
        (b) Full controllability rank = nx
        (c) Finite Kalman DARE solution
        (d) Finite LQR DARE solution

    Parameters
    ----------
    nx, nu, ny : state / input / output dimensions (must satisfy ny <= nx, nu <= nx)
    perturb    : relative noise level for A and B perturbations
    """
    rng = np.random.default_rng(seed)

    # Nominal A: diagonal with one unstable mode + mild coupling
    A_nom = np.zeros((nx, nx))
    eig_vals = np.linspace(0.8, 1.1, nx)[::-1]   # [1.1, ..., 0.8]
    for k in range(nx):
        A_nom[k, k] = eig_vals[k]
    for k in range(nx - 1):
        A_nom[k, k+1] = 0.1    # off-diagonal coupling

    # Nominal B: actuate the first nu states directly
    B_nom = np.zeros((nx, nu))
    B_nom[:nu, :nu] = np.eye(nu)

    # Nominal C: observe the first ny states directly
    C_nom = np.zeros((ny, nx))
    C_nom[:ny, :ny] = np.eye(ny)

    tasks: List[LQGTask] = []
    tries = 0
    while len(tasks) < M and tries < M * 50:
        tries += 1
        A = A_nom + perturb * rng.standard_normal((nx, nx))
        B = B_nom + perturb * rng.standard_normal((nx, nu))
        # C: small perturbation only — large perturbation can destroy observability
        C = C_nom + 0.02 * perturb * rng.standard_normal((ny, nx))

        # (a) Observability check
        obs  = np.vstack([C @ np.linalg.matrix_power(A, k) for k in range(nx)])
        if np.linalg.matrix_rank(obs) < nx:
            continue

        # (b) Controllability check
        ctrl = np.hstack([np.linalg.matrix_power(A, k) @ B for k in range(nx)])
        if np.linalg.matrix_rank(ctrl) < nx:
            continue
        ...
        try:
            if solve_discrete_are is not None:
                solve_discrete_are(A, B, C.T @ Qy @ C, R)
        except Exception:
            continue
    ...
    if len(tasks) < M:
        raise RuntimeError(
            f"Could not generate {M} feasible synthetic tasks. "
            "Try reducing perturb or adjusting nx/nu/ny.")
    return tasks
...
if __name__ == "__main__":
    # ---------------------------------------------------------------
    # Choose which system to run by setting SYSTEM:
    #   "cartpole"  — 4-state cartpole        (nx=4, nu=1, ny=2)
    #   "pendulum"  — 2-state inv. pendulum   (nx=2, nu=1, ny=1)
    #   "synthetic" — 4-state synthetic LTI   (nx=4, nu=2, ny=2)
    # ---------------------------------------------------------------
    SYSTEM = "pendulum"   # <-- change this to switch systems
    ...
    if SYSTEM == "cartpole":
        tasks  = make_demo_tasks(
            M=M, seed=1, perturb=0.05,
            m_p=0.1, m_c=1.0, ell=0.5, g=9.81, dt=0.05)
        prefix = "cartpole"

    elif SYSTEM == "pendulum":
        tasks  = make_pendulum_tasks(
            M=M, seed=1, perturb=0.05,
    ...
    print(f"  Tasks: {M},  nx={tasks[0].A.shape[0]}, "
          f"nu={tasks[0].B.shape[1]}, ny={tasks[0].C.shape[0]}")
    print(f"  Lifted dim: p*(nu+ny) = "
          f"{p*(tasks[0].B.shape[1]+tasks[0].C.shape[0])}")

    out = train_multitask_shared_Ktilde(
        tasks, p=p, iters=iters, eta=eta,
        verbose_every=2000, backtrack=True,
        compute_bisim=True, log_bisim=True, bisim_every=200,
        bisim_solver="AUTO")

    plot_task_costs(out["J_hist"], out["J_avg"], tasks, out["caches"],
                    f"fig_{prefix}_optimality_gap.pdf")
    plot_bisim_task_measures(out, f"fig_{prefix}_bisim_task_measures.pdf")
    plot_bisim_max_measure(out,   f"fig_{prefix}_bisim_max.pdf")
    save_run_data(out, prefix=f"run_{prefix}")